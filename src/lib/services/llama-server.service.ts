...
export class LlamaServerService {
	static readonly DEFAULT_URL = 'http://localhost:8080';
	...
	static label(base: string): string {
		try {
			...
			return LlamaServerService.isLoopback(base)
	...
	static async probe(base: string): Promise<boolean> {
		try {
			...
			if (LlamaServerService.isLoopback(base)) init.targetAddressSpace = 'loopback';
